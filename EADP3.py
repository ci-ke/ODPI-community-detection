...
def calculate_cc_ij(nodei, nodej, V_ij=None, maxw=1.0, t=0.2):
    if V_ij is None:
        V_ij = nx.common_neighbors(GG, nodei, nodej)
    r = 1.0  # 暂定1.0，todo 没有弄明白论文中的r所指的意思?????
    res = 0.0
    for node in V_ij:
        w_ipj = min(GG[nodei][node]['weight'], GG[nodej][node]['weight'])
...
def calculate_node_outgoing_weight(node):
    res = 0.0
    for n in GG.neighbors(node):
        res = res + GG[node][n]['weight']
    return res
...
def calculate_ls_ij(nodei, nodej, maxw=1.0, cc_weight=0.5, tt_weight=2, t=0.2):
    N_i = list(nx.neighbors(GG, nodei))
    N_j = list(nx.neighbors(GG, nodej))
    V_ij = list(nx.common_neighbors(GG, nodei, nodej))
    cc_ij = calculate_cc_ij(nodei, nodej, V_ij, maxw, t)
    t = (float(len(V_ij))) / len(set(N_i) | set(N_j)) + 1.0
    # i,j之间有边连接
    if GG.has_edge(nodei, nodej):
        A_ij = GG[nodei][nodej]['weight']
    else:
        A_ij = 0.0
    I_i = node_outgoing_weight_dictt[nodei]
    I_j = node_outgoing_weight_dictt[nodej]
...
def calculate_dist_ij(nodei, nodej, maxw=1.0, cc_weight=0.5, tt_weight=2, t=0.2):
    ...
    if GG.degree(nodei) == 0 or GG.degree(nodej) == 0:
        ls_ij = 0.0
    else:
...
def calculate_node_t_k(nodei):
    knn_nodes = nx.neighbors(GG, nodei)
    try:
        return max(
...
def calculate_dc(dc_weight=0.1):
    ...
    dc = math.sqrt(temp / (len(GG.nodes) - 1)) + u_k
    return dc * dc_weight
...
def init_dist_martix(cc_weight=0.5, tt_weight=2, t=0.2, need_calcualte_maxw=False):
    ...
    # a = np.zeros([n+1, n+1])
    nodes = sorted(list(GG.nodes))
    maxw = calculate_maxw(need_calcualte_maxw)
    for i in range(0, n):
        nodei = nodes[i]
        if GG.degree(nodei) == 0:
            continue
...
def calculate_knn():
    sum = 0
    # 返回的是每个节点的度的情况
    node_degree_tuple = nx.degree(GG)
...
def calculate_node_knn_neighbor(nodei, knn):
    ...
    # knn_nodes = [node for node in GG.nodes if node != nodei]
    # 得到节点的所有邻居节点之间的dist
...
def calculate_nodep(node, dc=0.2):
    ...
    knn_neighbors = list(GG.neighbors(node))
    res = 0.0
    # 如果不够就取所有的
...
def init_all_nodes_info(node_g_weight=2, dc_weight=0.1, dc=None):
    ...
    for node in GG.nodes:
        ...
        node_w = node_outgoing_weight_dictt[node]
        t = NodeInfo()
        t.node = node
        t.node_p = node_p
        t.node_w = node_w
...
def divide_ls_zero_node(
    ...
    if len(lg_node_p_list) == 0:
        ...
        # 先看它的邻居分配情况
        node_neighbors = nx.neighbors(GG, node)
...
def divide_ls_zero_node_2(
    ls_zero_nodes, GG_temp, node_community_dict, center_node_dict, all_nodes_info_dict
):
        ...
        for center_node in list(center_node_dict.keys()):
            try:
                shortest_path_length = nx.shortest_path_length(
                    GG_temp, source=ls_zero_node, target=center_node, weight='weight'
                )
...
def first_step(center_node_dict, all_nodes_info_dict):
    ...
    ls_zero_nodes = calculate_zeor_ls_with_center_node(
        list(node_community_dict.keys()), list(GG.nodes)
    )
    ...
    for node_info in all_nodes_info_list:
        ...
        if not node_info.is_center_node and waiting_node not in ls_zero_nodes:
            ...
            for node in list(center_node_dict.keys()):
                node_ij_weight = 0.0
                if GG.has_edge(waiting_node, node):
                    node_ij_weight = GG[waiting_node][node]['weight']
    ...
    # GG_temp = nx.from_numpy_array(dist_martix_2)
    # divide_ls_zero_node_2(ls_zero_nodes, GG_temp, node_community_dict,  center_node_dict, all_nodes_info_dict)
...
def second_step(
    ...
    for node_info in all_nodes_info_list:
        ...
        if not node_info.is_center_node:
            # # 计算该节点是否为包络节点
            # node_neighbors = list(nx.neighbors(GG, nodei))
...
def calculate_node_KN(NB_i, nodei):
    ...
    while len(NB_i) > 0 and t < 2:
        ...
        for node in NB_i:
            NB_i_dict[node] = list(nx.common_neighbors(GG, node, nodei))
        ni_kN = None
        ni_kN_size = -100
...
# 找到候选的重叠节点
def find_overlapping_candidates(GG, u, true_overlapping_nodes=[]):
    ...
    for node in list(GG.nodes):
        NB_i = list(nx.neighbors(GG, node))
        if len(NB_i) == 0:
            continue
...
def rebalace_G_txt(GG):
    ...
    for edge in GG.edges:
        GG_temp.add_edge(nodes_dict[int(edge[0])], nodes_dict[int(edge[1])])
    return GG_temp


def start2(
    param, G, node_outgoing_weight_dict, run_windows_lfr=False, new_makdir="test", dc=2
):
    ...
    global GG, dist_martix, ls_martix, dist_martix_2
    global node_outgoing_weight_dictt, node_knn_neighbors_dict
    global all_nodes_info_list
    global u, node_influence_dict
    global path

    GG = G
    node_outgoing_weight_dictt = node_outgoing_weight_dict
    ...
    overlapping_candidates, special_candidate_nodes = find_overlapping_candidates(
        GG, param.u, result.true_overlapping_nodes
    )
    ...
    community_nodes_dict, not_overlapping_community_node_dict = handle_result_to_txt2(
        all_nodes_info_list,
...
def deal_contrast_generate_image(new_makdir, param_dict, result_image_path, steps):
    ...
    for control_variable in control_variables:
        ...
        for param in params:
            ...
            step_results = []
            for i in range(0, steps):
                result, _ = start2(param, new_makdir=new_makdir)
                step_results.append(result)
                print("EADP i = {}, onmi={}".format(i, result.onmi))
                print("EADP i = {}, omega={}".format(i, result.omega))
                print("EADP i = {}, f1_score={}".format(i, result.f1_score))
...
def run_true_datasets():
    ...
    for known in knowns:
        ...
        result, need_print_result = start2(param)
    ...
    for test_dataset in test_datasets:
        ...
        for dc in dcs:
            ...
            if test_dataset.endswith(".gml"):
                ...
                param.dataset_type = "txt"
            result, need_print_result = start2(param, dc=dc)
...
def run_test_dc_for_lfr():
    ...
    for param in params:
        ...
        f1_score_result = []
        for dc in dcs:
            result, _ = start2(param=param, new_makdir="test_dc", dc=dc)
...
def run_test_u_for_lfr():
    ...
    for param in params:
        file_handle.write(str(param) + "\n")
        result, _ = start2(param=param, new_makdir="test_u")
...
if __name__ == '__main__':
    ...
    if run_platform == "linux":  # 表示是跑lfr网络生成数据的实验
        ...
        result, need_print_result = start2(param, run_windows_lfr, dc=None)
