...
def calculate_ls_ij(nodei, nodej, maxw=1.0, cc_weight=0.5, tt_weight=2, t=0.2):
    ...
    res = ((cc_ij * cc_weight + A_ij + t * tt_weight) * (len(V_ij) + 1)) / min(I_i, I_j)
    return res


# 计算节点i,j的distance
def calculate_dist_ij(nodei, nodej, maxw=1.0, cc_weight=0.5, tt_weight=2, t=0.2):
...
def calculate_node_t_k(nodei):
    ...
    try:
        return max(
            [dist_martix[nodei][nodej] for nodej in knn_nodes if dist_martix != 1 / b]
        )
    except:
        return 0.0
...
def init_dist_martix(cc_weight=0.5, tt_weight=2, t=0.2, need_calcualte_maxw=False):
    n = len(G.nodes)
    ...
    for i in range(0, n):
        ...
        for j in range(i + 1, n):
            ...
            if dist_martix[nodei][nodej] == 1 / b:
                dist_ij, ls_ij = calculate_dist_ij(
                    nodei, nodej, maxw, cc_weight, tt_weight, t
                )
...
def calculate_node_knn_neighbor(nodei, knn):
    ...
    # 对所有的邻居节点进行排序········································
    node_neighbors_dist_tuple_list = sorted(
        node_neighbors_dist_tuple_list, key=lambda x: x[1]
    )
    # 找到最小的k个邻居节点
    res = []
...
def init_all_nodes_info(node_g_weight=2, dc_weight=0.1, dc=None):
    ...
    for i in range(len(res)):
        ...
        if i == len(res) - 1:
            ...
            node_list = [
                res[x].node
                for x in range(i + 1, len(res))
                if res[x].node_p > node_info.node_p
            ]
...
def print_node_info():
    for node_info in all_nodes_info_list:
        print(
            "节点： { %s } node_p_1的值: { %f } node_g_1的值：{ %f } node_r的值： { %f } "
...
def filter_corredpond_nodes(all_nodes_info_list):
    ...
    for node_info in all_nodes_info_list:
        if (
            node_info.node_p < averge_eighty_percen_node_p
            or node_info.node_r < averge_eighty_percen_node_r
        ):
            pass
        else:
...
def select_center(node_info_list, averge_node_r, center_node_r_weight=0.8):
    ...
    while len(node_info_list) > 3:
        ...
        if (
            len(node_info_list) < 3
            or temp_node_info.node_dr < averge_node_r * center_node_r_weight
        ):
            break
...
def init_center_node(
    filter_nodes_info_list_index, filter_nodes_info_list, all_nodes_info_dict
):
    center_node_dict = {}
    community = 1
...
@timefn
def select_center_kmeans(node_info_list, all_nodes_info_dict, center_node_r_weight=1.0):
    ######################################
    #  使用聚类的方法进行中心节点的选择
    #  1）先使用KMeans进行二分聚类
    ...
    averge_bad_node_r = sum([node_info.node_r for node_info in node_info_list]) / len(
        bad_list
    )
...
def divide_ls_zero_node(
    node, all_nodes_info_list, node_community_dict, center_nodes_community
):
    index = 0
        ...
        for node_neighbor in node_neighbors:
            ...
            if t != -1:
                if t in node_neighbors_community_dict:
                    node_neighbors_community_dict[t] = (
                        node_neighbors_community_dict[t]
...
def divide_ls_zero_node_2(
    ls_zero_nodes, G_temp, node_community_dict, center_node_dict, all_nodes_info_dict
):
    ...
    for ls_zero_node in ls_zero_nodes:
        ...
        for center_node in list(center_node_dict.keys()):
            try:
                shortest_path_length = nx.shortest_path_length(
                    G_temp, source=ls_zero_node, target=center_node, weight='weight'
                )
        ...
        if (
            len(zero_with_center_node_dist_dict) == 0
...
def calculate_node_knn_neighboor_ls(
    nodei, knn_node_neighbors, node_community_dict, comminity=None
):
    res = 0.0
...
def calculate_node_membership(nodei, node_community_dict):
    ...
    for community_c in list(node_knn_community_to_node_dict.keys()):
        ...
        for nodej in node_knn_c:
            ...
            a = calculate_node_knn_neighboor_ls(
                nodej, nodej_knn, node_community_dict, community_c
            )
...
@timefn
def second_step(
    node_community_dict, c, enveloped_weight=0.5, overlapping_candidates=[]
):
    not_enveloped_nodes = []
        ...
        if not node_info.is_center_node:
            ...
            if not node_info.is_enveloped_node:
                ...
                if community not in node_knn_neighbors_community:
                    ...
                    node_membership_dict = calculate_node_membership(
                        nodei, node_community_dict
                    )
...
def handle_result_to_txt(
    all_nodes_info_list, not_overlapping_community_dict, new_makdir, run_platorm, param
):
    ...
    write_result_to_lfr_code_txt(new_makdir, community_nodes_dict, run_platorm, param)
    return community_nodes_dict, not_overlapping_community_node_dict


def write_result_to_lfr_code_txt(
    new_makdir, community_nodes_dict, run_platform="linux", param=None
):
    if run_platform == "linux":
        ...
        if param.run_ture_dataset:
            ...
            file_path = (
                path + "true_datasets/" + dataset + "_code.txt"
            )  # 生成真实数据集跑到的结果到相应的xxx_code.txt文件中，供后续的onmi等统计
    else:
        if param.run_ture_dataset and param.dataset is None:
            raise Exception("想要运行真实数据集的结果，真实数据集怎么为空了？？")
        dataset = param.dataset[0 : str(param.dataset).find(".")]
...
def calculate_ascending_nodes(filter_nodes_info_list, all_nodes_info_list):
    ...
    for i in range(
        len(all_nodes_info_list) - len(filter_nodes_info_list), len(all_nodes_info_list)
    ):
...
def start(param, run_windows_lfr=False, new_makdir="test", dc=None):
    ...
    dist_martix, ls_martix, dist_martix_2 = init_dist_martix(
        param.cc_weight, param.tt_weight, param.t, param.need_calcualte_maxw
    )
    ...
    all_nodes_info_dict = {
        node_info.node: node_info for node_info in all_nodes_info_list
    }
    ...
    # 非核心逻辑，不用管
    ascending_nod_p_nodes, ascending_nod_r_nodes = calculate_ascending_nodes(
        filter_nodes_info_list, all_nodes_info_list
    )
    ...
    if not use_kermaens:  # 使用原来的中心节点的方法
        ...
        filter_nodes_info_list_index = select_center(
            filter_nodes_info_list, averge_node_r, param.center_node_r_weight
        )
        ...
        center_node_dict = select_center_kmeans(
            filter_nodes_info_list, all_nodes_info_dict, param.center_node_r_weight
        )
    print("select center nodes end...")
    ...
    node_community_dict, ls_zero_nodes = first_step(
        center_node_dict, all_nodes_info_dict
    )
    ...
    overlapping_candidates, special_candidate_nodes = find_overlapping_candidates(
        G, param.u, result.true_overlapping_nodes
    )
    ...
    not_enveloped_nodes = second_step(
        node_community_dict, param.c, param.enveloped_weight, overlapping_candidates
    )
    ...
    community_nodes_dict, not_overlapping_community_node_dict = handle_result_to_txt(
        all_nodes_info_list,
        not_overlapping_node_community_dict,
        new_makdir,
        run_platform,
        param,
    )
    ...
    print(
        "overlapping_nodes find info: true, candidate, true&candidate, find, true&find"
    )
    print(
        len(result.true_overlapping_nodes),
        len(overlapping_candidates),
        len(set(result.true_overlapping_nodes) & set(overlapping_candidates)),
        len(find_overlapping_nodes),
        len(mapping_overlapping_nodes),
    )
    ...
    if run_platform == "linux" or param.run_ture_dataset:
        ...
        # from my_EQ4 import extended_modularity

        # from test import extended_modularity
        ...
        if param.is_known_dataset:
            ...
            # modularity = EQ.extended_modularity(G, path, new_makdir, true_dataset)
            ...
            result.modularity = modularity
            print("myDPC modularity{}".format(modularity))
        ...
        if param.need_calcualte_other_algorithm:
            ...
            write_result_to_lfr_code_txt(
                new_makdir, community_nodes_dict, run_platform=run_platform, param=param
            )
            if param.is_known_dataset:
                onmi = calculate_onmi(path, new_makdir, true_dataset)
                omega = calculate_omega(path, new_makdir, true_dataset)
                ...
                # modularity = EQ.extended_modularity(G, path, new_makdir, true_dataset)
                result.CPM_MODULARITY = modularity
                print("CPM modularity{}".format(modularity))
            cpm_end_time = time.time()
            # 2) EADP算法
            from EADP3 import start2

            eadp_start_time = time.time()
            print("start calculate EADP......")
            community_nodes_dict = start2(
                param,
                G,
                node_outgoing_weight_dict,
            ...
            if param.is_known_dataset:
                ...
                result.EADP_ONMI = onmi
                result.EADP_OMEGA = omega
                result.EADP_F1_SCORE = f1_score
                result.EADP_MODULARITY = modularity
            else:
                modularity = cal_modularity(G, path, new_makdir, true_dataset)
                # modularity = EQ.extended_modularity(G, path, new_makdir, true_dataset)

                result.EADP_MODULARITY = modularity
                print("EADP modularity{}".format(modularity))
            eadp_end_time = time.time()
            # 2) LFR_EX算法
            from my_LFM_EX import get_community_nodes_dict

            lfm_ex_start_time = time.time()
            ...
            try:
                ...
                print('something wrong ODPI#1294')
                pass
            if param.is_known_dataset:
                onmi = calculate_onmi(path, new_makdir, true_dataset)
                omega = calculate_omega(path, new_makdir, true_dataset)
                ...
                try:
                    ...
                    modularity = 0.0
                result.LFM_EX_MODULARITY = modularity
                print("LFM modularity{}".format(modularity))
            lfm_ex_end_time = time.time()
            # 3) SLPA标签传播算法(由于不稳定需要10次求得平均值)
            from my_SLPA import get_community_nodes_dict

            slpa_start_time = time.time()
            ...
            if param.is_known_dataset:
                ...
                modularity = float(sum(SLPA_MODULARITIES)) / count
                result.SLPA_MODULARITY = modularity
                print("SLPA modularity{}".format(modularity))
            slpa_end_time = time.time()
            ...
            if param.is_known_dataset:
                ...
                result.DEMON_MODULARITY = modularity
                print("DEMON modularity{}".format(modularity))
            demon_end_time = time.time()
            # 6) MOSES算法，由C++代码实现，
            ...
            if param.is_known_dataset:
                ...
                modularity = cal_modularity(G, path, new_makdir, true_dataset)
                # modularity = EQ.extended_modularity(G, path, new_makdir, true_dataset)
                print("MOSES modularity{}".format(modularity))
                result.MOSES_MODULARITY = modularity
            print("end calculate MOSES......")
            moses_end_time = time.time()
            if param.is_known_dataset:
                print(
                    "===========================onmi & omega & f1-score====================================="
                )
    ...
    # 统计一下时间而已，不重要
    end_time = time.time()
    spend_seconds = end_time - my_start_time
...
def run_linux_generate_picture(steps=5, new_makdir="test"):
    ...
    param_dict = {
        ...
        "node_contrast": generate_contrast,
    }  # 不同的N下的与其他5个对比算法的实验，生成对比图(目前不做)
    ...
    if new_makdir in [
        ...
        "mut_contrast",
        "node_contrast",
    ]:  # 表示的是处理的是与其他算法进行对比的情况(因为有些实验，只是针对自己的不同参数做一个对比而已的)
        deal_contrast_generate_image(new_makdir, param_dict, result_image_path, steps)
        return
    if new_makdir in [
        "muw_table",
        "mut_table",
    ]:  # 表示的是处理的实验仅仅一个与其他算法的一个对比实验的表格数据而已，和上面的那些得到对比图的不一样
            ...
            for for_2 in for_1:
                ...
                for param in for_2:
                    ...
                    for i in range(0, steps):
                        start_time_0713 = time.time()
                        result, _ = start(param, new_makdir=new_makdir)
                        end_time_0713 = time.time()
                        print('this time: ' + str(end_time_0713 - start_time_0713))
                        ...
                        print("ODPI i = {}, onmi={}".format(i, result.onmi))
                        print("ODPI i = {}, omega={}".format(i, result.omega))
                        print("ODPI i = {}, f1_score={}".format(i, result.f1_score))
                    ...
                    y_train_i.append(
                        str(evaluation_result.onmi)
                        + ","
                        + str(evaluation_result.omega)
                        + ","
                        + str(evaluation_result.f1_score)
                    )
                    print('-' * 30)
...
def deal_contrast_generate_image(new_makdir, param_dict, result_image_path, steps):
    ...
    for control_variable in control_variables:
        if new_makdir == "muw_contrast":
            params, show_image_params = param_dict.get(new_makdir)(muw=control_variable)
        elif new_makdir == "mut_contrast":
            params, show_image_params = param_dict.get(new_makdir)(mut=control_variable)
        elif new_makdir == "node_contrast":
            params, show_image_params = param_dict.get(new_makdir)(n=control_variable)
        else:
            return
        for show_image_param in show_image_params:
            show_image_param.result_image_path = result_image_path
        ...
        for param in params:
            ...
            for i in range(0, steps):
                ...
                if param.need_calcualte_other_algorithm:  # 讲道理走到这个方法来了，这里一般都为True
                    ...
                    print("EADP i = {}, onmi={}".format(i, result.EADP_ONMI))
                    print("EADP i = {}, omega={}".format(i, result.EADP_OMEGA))
                    print("EADP i = {}, f1_score={}".format(i, result.EADP_F1_SCORE))
                    print("LFM_EX i = {}, onmi={}".format(i, result.LFM_EX_ONMI))
                    print("LFM_EX i = {}, omega={}".format(i, result.LFM_EX_OMEGA))
                    print(
                        "LFM_EX i = {}, f1_score={}".format(i, result.LFM_EX_F1_SCORE)
                    )
            ...
            cpm.append(
                str(evaluation_result.CPM_ONMI)
                + ","
                + str(evaluation_result.CPM_OMEGA)
                + ","
                + str(evaluation_result.CPM_F1_SCORE)
            )
            eadp.append(
                str(evaluation_result.EADP_ONMI)
                + ","
                + str(evaluation_result.EADP_OMEGA)
                + ","
                + str(evaluation_result.EADP_F1_SCORE)
            )
            slpa.append(
                str(evaluation_result.SLPA_ONMI)
                + ","
                + str(evaluation_result.SLPA_OMEGA)
                + ","
                + str(evaluation_result.SLPA_F1_SCORE)
            )
            lfm_ex.append(
                str(evaluation_result.LFM_EX_ONMI)
                + ","
                + str(evaluation_result.LFM_EX_OMEGA)
                + ","
                + str(evaluation_result.LFM_EX_F1_SCORE)
            )
            gce.append(
                str(evaluation_result.GCE_ONMI)
                + ","
                + str(evaluation_result.GCE_OMEGA)
                + ","
                + str(evaluation_result.GCE_F1_SCORE)
            )
            demon.append(
                str(evaluation_result.DEMON_ONMI)
                + ","
                + str(evaluation_result.DEMON_OMEGA)
                + ","
                + str(evaluation_result.DEMON_F1_SCORE)
            )
            moses.append(
                str(evaluation_result.MOSES_ONMI)
                + ","
                + str(evaluation_result.MOSES_OMEGA)
                + ","
                + str(evaluation_result.MOSES_F1_SCORE)
            )
        y_trains.extend([my_dpc, cpm, eadp, slpa, lfm_ex, demon, moses])
        for x in y_trains:
            print(x)
...
def deal_contrast_generate_table(new_makdir, steps):
    ...
    for i in range(len(my_dpc)):
        ...
        onmis = [
            str(cpm_results[0]),
            str(eadp_results[0]),
            str(lfm_ex_results[0]),
            str(slpa_results[0]),
            str(demon_results[0]),
            str(moses_results[0]),
            str(my_dpc_results[0]),
        ]
        omega = [
            str(cpm_results[1]),
            str(eadp_results[1]),
            str(lfm_ex_results[1]),
            str(slpa_results[1]),
            str(demon_results[1]),
            str(moses_results[1]),
            str(my_dpc_results[1]),
        ]
        f1_score = [
            str(cpm_results[2]),
            str(eadp_results[2]),
            str(lfm_ex_results[2]),
            str(slpa_results[2]),
            str(demon_results[2]),
            str(moses_results[2]),
            str(my_dpc_results[2]),
        ]
        ...
        file_handle.write(
            "nodes: {}, muw: {} , mut: {} \n".format(
                str(params[i].n), str(params[i].muw), str(params[i].mut)
            )
        )
...
def run_test_u_for_lfr():
    ...
    for param in params:
        ...
        s = "true: {}, candidate: {}, special_candidate: {}, true&candidate: {}, find: {}, true&find {}".format(
            len(result.true_overlapping_nodes),
            len(result.overlapping_candidates),
            len(result.special_candidate_nodes),
            len(
                set(result.true_overlapping_nodes) & set(result.overlapping_candidates)
            ),
