...
class CommunityEX:
    ...
    def whether_add(self, node, node2comms):
        ...
        if new_fitness - old_fitness > theta:
            ...
            score = (
                1.0 * (x / max_d_k_in) * alpha
                + 1.0 * (y / degree_u) * beta
                - 1.0 * (z / degree_u) * (1 - alpha - beta)
            )
            return score
        else:
            return None
...
class LFM_EX:
        ...
        while len(node_not_include) != 0:
            ...
            while to_be_examined:
                ...
                for node in to_be_examined:
                    ...
                    m[node] = is_add
                m = {k: v for k, v in list(m.items()) if v is not None}
                # fitness增量全小于等于0，停止
                if len(m) == 0:
                    break
                to_be_add = sorted(list(m.items()), key=lambda x: x[1], reverse=True)[0]
    ...
    def merge(self, max_iter=100):
        ...
        while not done and count < max_iter:
            ...
            for root_i in list(partition.keys()):
                neigh_dict = adj_comms[root_i]
                # j 都是根节点
                max_sim = 0
                max_root_j = None
                for root_j, num_edge in list(neigh_dict.items()):
            ...
            log_key = set()
            merge_comms_biarrow = {}
            for k, v in list(merge_comms_single_arrow.items()):
                if (not v in log_key) and merge_comms_single_arrow[v] == k:
                    merge_comms_biarrow[k] = v
                    # 记录合并过的键值，防止重复合并
                    log_key.add(k)
            for w, u in list(merge_comms_biarrow.items()):
        ...
        result_comms = [list(v) for k, v in list(uf.get_partition().items())]
        return result_comms
    ...
    def is_overlapping(comms):
        ...
        for k, v in list(vertex_community.items()):
            if len(v) != 1:
                return True
        return False
...
def get_community_nodes_dict(G):
    algorithm = LFM_EX(G, 0.01, 0)
